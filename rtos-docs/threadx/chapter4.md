...
---
# <a name="chapter-4---description-of-azure-rtos-threadx-services"></a>Rozdział 4 — Opis usług Azure RTO ThreadX Services
...
- **block_ptr** <br>Wskaźnik do docelowego wskaźnika bloku. Po pomyślnym przydzieleniu adres przydzielonego bloku pamięci jest umieszczany w miejscu, w którym wskazuje ten parametr.
- **wait_option** <br>Definiuje, w jaki sposób działa usługa, jeśli nie ma dostępnych bloków pamięci. Opcje oczekiwania są zdefiniowane w następujący sposób:
  - **TX_NO_WAIT** (0x00000000) — wybranie **TX_NO_WAIT** powoduje natychmiastowe zwrócenie z tej usługi niezależnie od tego, czy zakończyło się pomyślnie, czy nie. *Jest to jedyna prawidłowa opcja, jeśli usługa jest wywoływana z niebędącego wątkiem, np. inicjalizacji, czasomierza lub ISR*.
  - **TX_WAIT_FOREVER** (0xFFFFFFF) — wybranie **TX_WAIT_FOREVER** powoduje, że wątek wywołujący zawiesza się w nieskończoność do momentu udostępnienia bloku pamięci.
  - *wartość limitu czasu* (0X00000001 przez 0xFFFFFFFE) — wybranie wartości liczbowej (1-0xFFFFFFFE) określa maksymalną liczbę cykli czasomierza, która ma zostać wstrzymana podczas oczekiwania na blok pamięci.

### <a name="return-values"></a>Wartości zwrócone

- **TX_SUCCESS**    (0x00) pomyślna alokacja bloku pamięci.
- Pula bloków pamięci **TX_DELETED** (0x01) została usunięta podczas wstrzymania wątku.
- Usługa **TX_NO_MEMORY** (0x10) nie może przydzielić bloku pamięci w określonym czasie, aby oczekiwać.
- Zawieszenie **TX_WAIT_ABORTED** (0x1A) zostało przerwane przez inny wątek, CZASOMIERZ lub ISR.
...
- **name_ptr**  Wskaźnik na nazwę puli bloków pamięci.
...
- Rozmiar **TX_SIZE_ERROR** (0x05) puli jest nieprawidłowy.
...
- **pool_ptr**  Wskaźnik do wcześniej utworzonej puli bloków pamięci.
- **Nazwa**  Wskaźnik do miejsca docelowego dla wskaźnika do nazwy puli bloku.
...
- **TX_SUCCESS** (0X00) pomyślne pobranie informacji o puli blokad.
- **TX_POOL_ERROR** (0X02) Nieprawidłowy wskaźnik puli bloków pamięci.
...
> [!IMPORTANT]
> Aby można było zwracać informacje o wydajności *, biblioteka i aplikacja ThreadX muszą zostać skompilowane z użyciem* **TX_BLOCK_POOL_ENABLE_PERFORMANCE_INFO** *zdefiniowanej dla tej usługi.*

### <a name="parameters"></a>Parametry

- **pool_ptr**  Wskaźnik do wcześniej utworzonej puli bloków pamięci.
- **alokuje** Wskaźnik do miejsca docelowego dla liczby żądań alokacji wykonanych w tej puli.
- **wersje**  Wskaźnik do miejsca docelowego dla liczby żądań wydania wykonanych w tej puli.
- **zawieszenie**   Wskaźnik do miejsca docelowego dla liczby zawieszeń alokacji wątku w tej puli.
- **limity czasu**  Wskaźnik do miejsca docelowego dla liczby limitów czasu zawieszenia przydziału w tej puli.
...
- **TX_SUCCESS**    (0X00) pomyślne uzyskanie wydajności puli blokowych.
...
- **alokuje** Wskaźnik do miejsca docelowego dla łącznej liczby żądań alokacji wykonanych dla wszystkich pul bloku.
- **wersje**  Wskaźnik do miejsca docelowego dla łącznej liczby żądań zwolnienia wykonanych na wszystkich pulach bloku.
- **zawieszenie**   Wskaźnik do miejsca docelowego dla łącznej liczby zawieszeń alokacji wątku dla wszystkich pul bloków.
...
- **TX_SUCCESS** (0X00) pomyślnie priorytetyzacja puli zablokowanych bloków.
- **TX_POOL_ERROR** (0X02) Nieprawidłowy wskaźnik puli bloków pamięci.
...
- **pool_ptr** <br>Wskaźnik do wcześniej utworzonej puli bloków pamięci.
- **memory_ptr** <br>Wskaźnik na wskaźnik pamięci docelowej. Po pomyślnym przydzieleniu adres przydzielonego obszaru pamięci jest umieszczany w miejscu, do którego wskazuje ten parametr.
- **memory_size** <br>Liczba żądanych bajtów.
- **wait_option** <br>Definiuje, w jaki sposób działa usługa, jeśli nie jest dostępna wystarczająca ilość pamięci. Opcje oczekiwania są zdefiniowane w następujący sposób:
  - **TX_NO_WAIT** (0x00000000) — wybranie **TX_NO_WAIT** powoduje natychmiastowe zwrócenie z tej usługi niezależnie od tego, czy zakończyło się pomyślnie. *Jest to jedyna prawidłowa opcja, jeśli usługa jest wywoływana z inicjacji.*
  - **TX_WAIT_FOREVER** 0xffffffff) — wybranie **TX_WAIT_FOREVER** powoduje, że wątek wywołujący zawiesza się przez nieograniczony czas do momentu udostępnienia wystarczającej ilości pamięci.
  - *wartość limitu czasu* (0X00000001 przez 0xFFFFFFFE) — wybranie wartości liczbowej (1-0xFFFFFFFE) określa maksymalną liczbę cykli czasomierza, która ma zostać wstrzymana podczas oczekiwania na pamięć.

### <a name="return-values"></a>Wartości zwrócone

- Pomyślna alokacja pamięci **TX_SUCCESS** (0x00).
- Pula pamięci **TX_DELETED** (0x01) została usunięta podczas wstrzymania wątku.
- Usługa **TX_NO_MEMORY** (0x10) nie może przydzielić pamięci w określonym czasie, aby czekać.
...
- **TX_PTR_ERROR** (0X03) Nieprawidłowy wskaźnik do wskaźnika docelowego.
- Żądany rozmiar **TX_SIZE_ERROR** (0X05) jest równy zero lub większy niż Pula.
- **TX_WAIT_ERROR** (0x04) opcja oczekiwania inna niż TX_NO_WAIT została określona w wywołaniu z niewątku.
- **TX_CALLER_ERROR** (0X13) Nieprawidłowy obiekt wywołujący tej usługi.
...
- **name_ptr** Wskaźnik na nazwę puli pamięci.
- **pool_start** Adres początkowy puli pamięci. Adres początkowy musi być wyrównany do rozmiaru typu danych ULONG.
...
- **pool_ptr** Wskaźnik do wcześniej utworzonej puli pamięci.
- **Nazwa** Wskaźnik do miejsca docelowego dla wskaźnika do nazwy puli bajtów.
...
- **first_suspended** Wskaźnik do miejsca docelowego dla wskaźnika do wątku, który jest pierwszy na liście zawieszeń tej puli bajtów.
- **suspended_count** Wskaźnik do miejsca docelowego dla liczby aktualnie zawieszonych wątków w tej puli bajtów.
...
> [!IMPORTANT]
> Aby można było zwracać informacje o wydajności *, biblioteka i aplikacja ThreadX muszą zostać skompilowane z użyciem* **TX_BYTE_POOL_ENABLE_PERFORMANCE_INFO** *zdefiniowanej dla tej usługi.*
...
- **podziały** Wskaźnik do miejsca docelowego dla liczby wewnętrznych bloków pamięci podzielonej (fragmenty) utworzonych podczas żądania alokacji w tej puli.
- **zawieszenie** Wskaźnik do miejsca docelowego dla liczby zawieszeń alokacji wątku w tej puli.
- **limity czasu** Wskaźnik do miejsca docelowego dla liczby limitów czasu zawieszenia przydziału w tej puli.
...
- **podziały** Wskaźnik do miejsca docelowego dla łącznej liczby bloków pamięci wewnętrznej (fragmentów) utworzonych podczas żądania alokacji dla wszystkich pul bajtów.
- **zawieszenie** Wskaźnik do miejsca docelowego dla łącznej liczby zawieszeń alokacji wątku dla wszystkich pul bajtów.
...
- **name_ptr** Wskaźnik na nazwę grupy flag zdarzeń.

### <a name="return-values"></a>Wartości zwrócone

- Pomyślna operacja tworzenia grupy zdarzeń **TX_SUCCESS** (0x00).
...
- Usuwanie grupy flag zdarzeń zakończonych niepowodzeniem **TX_SUCCESS** (0x00).
...
- **requested_flags** <br>32-bitowa zmienna bez znaku reprezentująca żądane flagi zdarzenia.
- **get_option** <br>Określa, czy są wymagane wszystkie lub wszystkie wymagane flagi zdarzeń. Następujące opcje są prawidłowe:
...
- **actual_flags_ptr** <br>Wskaźnik do miejsca docelowego, gdzie są umieszczane flagi pobranych zdarzeń. Należy zauważyć, że uzyskane bieżące flagi mogą zawierać flagi, które nie zostały zażądane.
- **wait_option**  <br>Definiuje, w jaki sposób działa usługa, jeśli wybrane flagi zdarzeń nie są ustawione. Opcje oczekiwania są zdefiniowane w następujący sposób:
  - **TX_NO_WAIT** (0x00000000) — wybranie TX_NO_WAIT powoduje natychmiastowe zwrócenie z tej usługi niezależnie od tego, czy zakończyło się pomyślnie. Jest to jedyna prawidłowa opcja, jeśli usługa jest wywoływana z niewątku; np. Inicjalizacja, czasomierz lub ISR.
  - **TX_WAIT_FOREVER** wartość limitu czasu (0xffffffff) — wybranie TX_WAIT_FOREVER powoduje, że wątek wywołujący zawiesza się przez nieograniczony czas do momentu udostępnienia flag zdarzeń.
  - wartość limitu czasu (0x00000001 przez 0xFFFFFFFE) — wybranie wartości liczbowej (1-0xFFFFFFFE) określa maksymalną liczbę cykli czasomierza, która ma pozostać zawieszona podczas oczekiwania na flagi zdarzenia.

### <a name="return-values"></a>Wartości zwrócone

- Pomyślne flagi zdarzeń **TX_SUCCESS** (0x00) get.
- Grupa flag zdarzeń **TX_DELETED** (0x01) została usunięta podczas wstrzymania wątku.
- Usługa **TX_NO_EVENTS** (0x07) nie może pobrać określonych zdarzeń w określonym czasie oczekiwania.
- Zawieszenie **TX_WAIT_ABORTED** (0x1A) zostało przerwane przez inny wątek, czasomierz lub proces ISR.
- **TX_GROUP_ERROR** (0X06) Nieprawidłowy wskaźnik grupy flag zdarzeń.
...
- **group_ptr** Wskaźnik do bloku sterowania grupą flag zdarzeń.
- **Nazwa** Wskaźnik do miejsca docelowego dla wskaźnika do nazwy grupy flag zdarzeń.
- **current_flags** Wskaźnik do miejsca docelowego dla bieżących flag ustawionych w grupie flag zdarzeń.
- **first_suspended** Wskaźnik do miejsca docelowego dla wskaźnika do wątku, który jest pierwszy na liście zawieszeń tej grupy flag zdarzeń.
- **suspended_count** Wskaźnik do miejsca docelowego dla liczby wątków, które są obecnie zawieszone w tej grupie flag zdarzeń.
- **next_group** Wskaźnik do miejsca docelowego dla wskaźnika następnej grupy flag zdarzeń.
...
> [!IMPORTANT]
> Aby można było zwracać informacje o wydajności *, biblioteka i aplikacja ThreadX muszą zostać skompilowane z użyciem* **TX_EVENT_FLAGS_ENABLE_PERFORMANCE_INFO** *zdefiniowanej dla tej usługi.*

### <a name="parameters"></a>Parametry

- **group_ptr** Wskaźnik do wcześniej utworzonej grupy flag zdarzeń.
- **zestawy** Wskaźnik do miejsca docelowego dla liczby flag zdarzeń ustawia żądania wykonane dla tej grupy.
- **Pobiera** Wskaźnik do miejsca docelowego dla liczby flag zdarzeń pobieranie żądań wykonanych dla tej grupy.
- **zawieszenie** Wskaźnik do miejsca docelowego dla liczby flag zdarzeń wątku pobieranie zawieszeń dla tej grupy.
...
- **zestawy** Wskaźnik do miejsca docelowego dla łącznej liczby flag zdarzeń ustawionych żądań wykonywanych dla wszystkich grup.
- **Pobiera** Wskaźnik do miejsca docelowego dla łącznej liczby flag zdarzeń pobieranie żądań wykonanych dla wszystkich grup.
- **zawieszenie** Wskaźnik do miejsca docelowego dla łącznej liczby flag zdarzeń wątku pobieranie zawieszeń dla wszystkich grup.
...
- **group_ptr** <br>Wskaźnik do wcześniej utworzonego bloku kontroli grupy flag zdarzeń.
- **flags_to_set** <br>Określa flagi zdarzeń do ustawienia lub wyczyszczenia w zależności od wybranej opcji zestawu.
- **set_option** <br>Określa, czy określone flagi zdarzenia są ANDed lub logicznie do bieżących flag zdarzenia w grupie. Następujące opcje są prawidłowe:
  - **TX_AND** (0x02)
  - **TX_OR** (0x00)

  Wybranie TX_AND określa, że określone flagi zdarzeń są **i** są wbudowane w bieżące flagi zdarzenia w grupie. Ta opcja jest często używana do czyszczenia flag zdarzeń w grupie. W przeciwnym razie, jeśli określono TX_OR, określone flagi zdarzeń są **lub** Ed bieżącym zdarzeniem w grupie.

### <a name="return-values"></a>Wartości zwrócone
- Ustawiono flagę zdarzenia pomyślnego **TX_SUCCESS** (0x00).
...
Ta usługa rejestruje funkcję wywołania zwrotnego powiadomienia, która jest wywoływana za każdym razem, gdy jedna lub więcej flag zdarzeń jest ustawionych w określonej grupie flag zdarzeń. Przetwarzanie wywołania zwrotnego powiadomienia jest definiowane przez
...
- **new_posture** Ten parametr określa, czy przerwania są wyłączone czy włączone. Wartości prawne obejmują **TX_INT_DISABLE** i **TX_INT_ENABLE**. Rzeczywiste wartości tych parametrów są specyficzne dla portów. Ponadto niektóre architektury przetwarzania mogą obsługiwać dodatkowe przerwania postures.

### <a name="return-values"></a>Wartości zwrócone
- **poprzedni stan** Ta usługa zwraca poprzednią stan przerwania do obiektu wywołującego. Dzięki temu użytkownicy usługi mogą przywrócić poprzednią stan po wyłączeniu przerwań.
...
- **name_ptr** Wskaźnik na nazwę obiektu mutex.
...
- **wait_option** <br>Definiuje sposób zachowania usługi, jeśli element mutex jest już własnością innego wątku. Opcje oczekiwania są zdefiniowane w następujący sposób:
  - **TX_NO_WAIT** (0x00000000) — wybranie TX_NO_WAIT powoduje natychmiastowe zwrócenie z tej usługi niezależnie od tego, czy zakończyło się pomyślnie. *Jest to jedyna prawidłowa opcja, jeśli usługa jest wywoływana z inicjacji.*
  - **TX_WAIT_FOREVER** wartość limitu czasu (0xffffffff) — wybranie **TX_WAIT_FOREVER** powoduje, że wątek wywołujący zawiesza się nieokreślony czas do momentu udostępnienia elementu mutex.
  - wartość limitu czasu (0x00000001 przez 0xFFFFFFFE) — wybranie wartości liczbowej (1-0xFFFFFFFE) określa maksymalną liczbę cykli czasomierza, która ma zostać wstrzymana podczas oczekiwania na mutex.

### <a name="return-values"></a>Wartości zwrócone

- **TX_SUCCESS** (0x00) pomyślna operacja pobrania obiektu mutex.
- Element mutex **TX_DELETED** (0x01) został usunięty podczas wstrzymania wątku.
- Usługa **TX_NOT_AVAILABLE** (0x1D) nie mogła pobrać własności obiektu mutex w określonym czasie, aby czekać.
- Zawieszenie **TX_WAIT_ABORTED** (0x1A) zostało przerwane przez inny wątek, czasomierz lub proces ISR.
- **TX_MUTEX_ERROR** (0X1C) Nieprawidłowy wskaźnik muteksu.
- **TX_WAIT_ERROR** (0x04) opcja oczekiwania inna niż TX_NO_WAIT została określona w wywołaniu z niewątku.
- **TX_CALLER_ERROR** (0X13) Nieprawidłowy obiekt wywołujący tej usługi.
...
- **mutex_ptr** Wskaźnik do bloku sterowania muteksem.
- **Nazwa** Wskaźnik do miejsca docelowego dla wskaźnika do nazwy obiektu mutex.
- **Liczba** Wskaźnik do miejsca docelowego dla liczby własności obiektu mutex.
- **właściciel** Wskaźnik do miejsca docelowego dla wskaźnika wątku będącego właścicielem.
- **first_suspended** Wskaźnik do miejsca docelowego dla wskaźnika do wątku, który jest pierwszy na liście zawieszeń tego elementu mutex.
- **suspended_count** Wskaźnik do miejsca docelowego dla liczby wątków, które są obecnie zawieszone dla tego obiektu mutex.
...
- **TX_SUCCESS** (0X00) pomyślne pobranie informacji o mutex.
- **TX_MUTEX_ERROR** (0X1C) Nieprawidłowy wskaźnik muteksu.
...
> [!IMPORTANT]
> *Biblioteka i aplikacja ThreadX muszą być kompilowane przy użyciu*  * **TX_MUTEX_ENABLE_PERFORMANCE_INFO** _ _defined do zwrócenia informacji o wydajności przez tę usługę *

### <a name="parameters"></a>Parametry

- **mutex_ptr** Wskaźnik do wcześniej utworzonego obiektu mutex.
- **umieszcza** Wskaźnik do miejsca docelowego dla liczby żądań PUT wykonanych dla tego obiektu mutex.
- **Pobiera** Wskaźnik do miejsca docelowego dla liczby żądań GET wykonanych dla tego obiektu mutex.
- **zawieszenie** Wskaźnik do miejsca docelowego dla liczby elementów mutex wątku Get zawieszeń dla tego obiektu mutex.
...
> [!IMPORTANT]
> Aby można było zwracać informacje o wydajności *, biblioteka i aplikacja ThreadX muszą zostać skompilowane z użyciem* **TX_MUTEX_ENABLE_PERFORMANCE_INFO** *zdefiniowanej dla tej usługi.*

### <a name="parameters"></a>Parametry

- **umieszcza** Wskaźnik do miejsca docelowego dla łącznej liczby żądań PUT wykonanych dla wszystkich muteksów.
- **Pobiera** Wskaźnik do miejsca docelowego dla łącznej liczby żądań GET wykonanych dla wszystkich muteksów.
- **zawieszenie** Wskaźnik do miejsca docelowego dla łącznej liczby zawieszeń elementu mutex wątku dla wszystkich muteksów.
- **limity czasu** Wskaźnik do miejsca docelowego dla łącznej liczby limitów czasu zawieszania dla wszystkich muteksów.
- **Inversions** Wskaźnik do miejsca docelowego dla łącznej liczby niewersji priorytetu wątku dla wszystkich muteksów.
...
- Element mutex **TX_NOT_OWNED** (0x1E) nie należy do obiektu wywołującego.
...
- Pomyślnie usunięto kolejkę komunikatów **TX_SUCCESS** (0x00).
...
Jeśli kolejka jest pełna, komunikaty wszystkich zawieszonych wątków są odrzucane. Każdy zawieszony wątek zostanie wznowiony ze stanem powrotu, który wskazuje, że wiadomość została wysłana pomyślnie. Jeśli kolejka jest pusta, ta usługa nie robi nic.
...
- Opróżnianie kolejki komunikatów **TX_SUCCESS** (0x00).
- **TX_QUEUE_ERROR** (0X09) Nieprawidłowy wskaźnik kolejki komunikatów.
...
- **queue_ptr** <br>Wskaźnik do bloku sterowania kolejki komunikatów.
- **source_ptr** <br>Wskaźnik na komunikat.
- **wait_option**  <br>Definiuje, w jaki sposób działa usługa, jeśli kolejka komunikatów jest pełna. Opcje oczekiwania są zdefiniowane w następujący sposób:
  - **TX_NO_WAIT** (0x00000000) — wybranie TX_NO_WAIT powoduje natychmiastowe zwrócenie z tej usługi niezależnie od tego, czy zakończyło się pomyślnie. *Jest to jedyna prawidłowa opcja, jeśli usługa jest wywoływana z niewątku; np. Inicjalizacja, czasomierz lub ISR.*
  - **TX_WAIT_FOREVER** (0xffffffff) — wybranie TX_WAIT_FOREVER powoduje, że wątek wywołujący zawiesza się w nieskończoność do momentu, w którym znajduje się w kolejce.
  - wartość limitu czasu (0x00000001 przez 0xFFFFFFFE) — wybranie wartości liczbowej (1-0xFFFFFFFE) określa maksymalną liczbę cykli czasomierza, która ma pozostać zawieszona podczas oczekiwania na pokój w kolejce.

### <a name="return-values"></a>Wartości zwrócone

- **TX_SUCCESS** (0X00) pomyślnie wysłano komunikat.
- Kolejka komunikatów **TX_DELETED** (0x01) została usunięta podczas wstrzymania wątku.
- Usługa **TX_QUEUE_FULL** (0x0B) nie mogła wysłać komunikatu, ponieważ kolejka była zapełniona przez czas oczekiwania przez określony czas.
- Zawieszenie **TX_WAIT_ABORTED** (0x1A) zostało przerwane przez inny wątek, czasomierz lub proces ISR.
- **TX_QUEUE_ERROR** (0X09) Nieprawidłowy wskaźnik kolejki komunikatów.
...
- **queue_ptr** Wskaźnik do wcześniej utworzonej kolejki komunikatów.
- **Nazwa** Wskaźnik do miejsca docelowego dla wskaźnika do nazwy kolejki.
- w **kolejce** Wskaźnik do miejsca docelowego dla liczby komunikatów aktualnie znajdujących się w kolejce.
...
- **first_suspended** Wskaźnik do miejsca docelowego dla wskaźnika do wątku, który jest pierwszy na liście zawieszeń tej kolejki.
...
- Informacje dotyczące kolejki powiodło się **TX_SUCCESS** (0x00).
- **TX_QUEUE_ERROR** (0X09) Nieprawidłowy wskaźnik kolejki komunikatów.
...
> [!IMPORTANT]
> *Biblioteka i aplikacja ThreadX muszą być kompilowane przy użyciu*  * **TX_QUEUE_ENABLE_PERFORMANCE_INFO** _ _defined do zwrócenia informacji o wydajności przez tę usługę *
...
- **messages_received** Wskaźnik do miejsca docelowego dla liczby żądań odebrania wykonanych dla tej kolejki.
...
- Pomyślna wydajność kolejki **TX_SUCCESS** (0x00).
...
- **TX_SUCCESS** (0X00) pomyślne pobieranie wydajności systemu z kolejki.
- **TX_FEATURE_NOT_ENABLED** (0xFF) system nie został skompilowany z włączonymi informacjami o wydajności.
...
Ta usługa umieszcza wątek o najwyższym priorytecie dla komunikatu (lub umieszcza komunikat) w tej kolejce na początku listy zawieszeń.

Wszystkie pozostałe wątki pozostają w tej samej kolejności FIFO, w której zostały zawieszone.
...
> [!IMPORTANT]
> *Określony obszar pamięci docelowej musi być wystarczająco duży, aby pomieścić komunikat; oznacza to, że miejsce docelowe komunikatu wskazywane przez*  * **destination_ptr** _ _must być co najmniej tak duże, jak rozmiar komunikatu dla tej kolejki. W przeciwnym razie, jeśli miejsce docelowe nie jest wystarczająco duże, uszkodzenie pamięci występuje w następującym obszarze pamięci. *

### <a name="parameters"></a>Parametry

- **queue_ptr** <br>Wskaźnik do wcześniej utworzonej kolejki komunikatów.
- **destination_ptr** <br>Lokalizacja, w której ma zostać skopiowana wiadomość.
- **wait_option** <br>Definiuje, w jaki sposób działa usługa, jeśli kolejka komunikatów jest pusta. Opcje oczekiwania są zdefiniowane w następujący sposób:
  - **TX_NO_WAIT** (0x00000000) — wybranie TX_NO_WAIT powoduje natychmiastowe zwrócenie z tej usługi niezależnie od tego, czy zakończyło się pomyślnie. Jest to jedyna prawidłowa opcja, jeśli usługa jest wywoływana z niewątku; np. Inicjalizacja, czasomierz lub ISR.
  - **TX_WAIT_FOREVER** (0xffffffff) — wybranie TX_WAIT_FOREVER powoduje, że wątek wywołujący zawiesza się w nieskończoność do momentu udostępnienia komunikatu.
  - wartość limitu czasu (0x00000001 przez 0xFFFFFFFE) — wybranie wartości liczbowej (1-0xFFFFFFFE) określa maksymalną liczbę cykli czasomierza, która ma zostać zawieszona podczas oczekiwania na komunikat.

### <a name="return-values"></a>Wartości zwrócone

- **TX_SUCCESS** (0X00) pomyślne pobranie komunikatu.
- Kolejka komunikatów **TX_DELETED** (0x01) została usunięta podczas wstrzymania wątku.
- Usługa **TX_QUEUE_EMPTY** (0x0A) nie może pobrać komunikatu, ponieważ kolejka była pusta przez czas oczekiwania przez określony czas.
- Zawieszenie **TX_WAIT_ABORTED** (0x1A) zostało przerwane przez inny wątek, czasomierz lub proces ISR.
- **TX_QUEUE_ERROR** (0X09) Nieprawidłowy wskaźnik kolejki komunikatów.
...
- **source_ptr** <br>Wskaźnik na komunikat.
- **wait_option** <br>Definiuje, w jaki sposób działa usługa, jeśli kolejka komunikatów jest pełna. Opcje oczekiwania są zdefiniowane w następujący sposób:
  - **TX_NO_WAIT** (0x00000000) — wybranie TX_NO_WAIT powoduje natychmiastowe zwrócenie z tej usługi niezależnie od tego, czy zakończyło się pomyślnie. *Jest to jedyna prawidłowa opcja, jeśli usługa jest wywoływana z niebędącego wątkiem, np. inicjalizacji, czasomierza lub ISR*.
  - **TX_WAIT_FOREVER** (0xffffffff) — wybranie TX_WAIT_FOREVER powoduje, że wątek wywołujący zawiesza się w nieskończoność do momentu, w którym znajduje się w kolejce.
  - wartość limitu czasu (0x00000001 przez 0xFFFFFFFE) — wybranie wartości liczbowej (1-0xFFFFFFFE) określa maksymalną liczbę cykli czasomierza, która ma pozostać zawieszona podczas oczekiwania na pokój w kolejce.
...
>[!NOTE]
> *Wywołanie zwrotne powiadomienia o wysłaniu kolejki aplikacji nie może wywołać żadnego interfejsu API ThreadX z opcją zawieszenia.*

### <a name="parameters"></a>Parametry

- **queue_ptr** Wskaźnik do wcześniej utworzonej kolejki.
- **queue_send_notify** Wskaźnik do funkcji powiadomień o wysłaniu kolejki aplikacji. Jeśli ta wartość jest TX_NULL, powiadomienie jest wyłączone.

### <a name="return-values"></a>Wartości zwrócone

- **TX_SUCCESS** (0X00) pomyślnie zarejestrowano powiadomienie o wysłaniu kolejki.
...
- **granica** Maksymalny dozwolony limit dla semafora (prawidłowe wartości mieszczą się w zakresie od 1 do 0xFFFFFFFF).

### <a name="return-values"></a>Wartości zwrócone

- **TX_SUCCESS (0x00)** Pomyślny pułap semafora został przesunięty.
- Żądanie Put **TX_CEILING_EXCEEDED** (0x21) przekracza limit.
...
- **name_ptr** Wskaźnik na nazwę semafora.
...
- **TX_SUCCESS** (0X00) pomyślnie zliczanie usunięcia semafora.
...
- **semaphore_ptr** <br>Wskaźnik do wcześniej utworzonego semafora zliczania.
- **wait_option** <br>Definiuje, w jaki sposób działa usługa, jeśli nie ma dostępnych wystąpień semafora; oznacza to, że liczba semaforów jest równa zero. Opcje oczekiwania są zdefiniowane w następujący sposób:
  - **TX_NO_WAIT** (0x00000000) — wybranie TX_NO_WAIT powoduje natychmiastowe zwrócenie z tej usługi niezależnie od tego, czy zakończyło się pomyślnie. *Jest to jedyna prawidłowa opcja, jeśli usługa jest wywoływana z niewątku; np. Inicjalizacja, czasomierz lub ISR.*
  - **TX_WAIT_FOREVER** (0xffffffff) — wybranie TX_WAIT_FOREVER powoduje, że wątek wywołujący zawiesza się w nieskończoność do momentu dostępności wystąpienia semafora.
  - wartość limitu czasu (0x00000001 przez 0xFFFFFFFE) — wybranie wartości liczbowej (1-0xFFFFFFFE) określa maksymalną liczbę cykli czasomierza, która ma zostać wstrzymana podczas oczekiwania na wystąpienie semafora.

### <a name="return-values"></a>Wartości zwrócone

- **TX_SUCCESS** (0X00) pomyślne pobranie wystąpienia semafora.
- Nie usunięto semafora zliczania **TX_DELETED** (0x01), gdy wątek został zawieszony.
- Usługa **TX_NO_INSTANCE** (0x0D) nie mogła pobrać wystąpienia semafora zliczania (liczba semaforów wynosi zero w określonym czasie oczekiwania).
- Zawieszenie **TX_WAIT_ABORTED** (0x1A) zostało przerwane przez inny wątek, czasomierz lub proces ISR.
- **TX_SEMAPHORE_ERROR** (0X0C) Nieprawidłowy wskaźnik zliczania semafora.
- **TX_WAIT_ERROR** (0x04) opcja oczekiwania inna niż TX_NO_WAIT została określona w wywołaniu z niewątku.
...
- **semaphore_ptr** Wskaźnik do bloku sterowania semaforem.
- **Nazwa** Wskaźnik do miejsca docelowego dla wskaźnika do nazwy semafora.
- **current_value** Wskaźnik do miejsca docelowego dla licznika bieżącego semafora.
- **first_suspended** Wskaźnik do miejsca docelowego dla wskaźnika do wątku, który jest pierwszy na liście zawieszeń tego semafora.
...
- Pobieranie informacji o **TX_SUCCESS** (0x00).
...
> [!IMPORTANT]
> *Biblioteka i aplikacja ThreadX muszą być kompilowane przy użyciu*  * **TX_SEMAPHORE_ENABLE_PERFORMANCE_INFO** _ _defined do zwrócenia informacji o wydajności przez tę usługę *

**Parametry**

-  **semaphore_ptr** Wskaźnik do wcześniej utworzonego semafora.
-  **umieszcza** Wskaźnik do miejsca docelowego dla liczby żądań PUT wykonanych dla tego semafora.
-  **Pobiera** Wskaźnik do miejsca docelowego dla liczby żądań GET wykonanych dla tego semafora.
-  **zawieszenie** Wskaźnik do miejsca docelowego dla liczby zawieszeń wątku dla tego semafora.
...
- **umieszcza** Wskaźnik do miejsca docelowego dla łącznej liczby żądań PUT wykonanych na wszystkich semaforach.
- **Pobiera** Wskaźnik do miejsca docelowego dla łącznej liczby żądań GET wykonanych na wszystkich semaforach.
- **zawieszenie** Wskaźnik do miejsca docelowego dla łącznej liczby zawieszeń wątku na wszystkich semaforach.
...
- Pobieranie wydajności systemu **TX_SUCCESS** (0x00).
- **TX_FEATURE_NOT_ENABLED** (0xFF) system nie został skompilowany z włączonymi informacjami o wydajności.
...
- Pomyślna priorytetyzacja semafora **TX_SUCCESS** (0x00).
- **TX_SEMAPHORE_ERROR** (0X0C) Nieprawidłowy wskaźnik zliczania semafora.
...
- **TX_SUCCESS** (0X00) zakończony powodzeniem semafora.
...
> [!NOTE]
> *Wywołanie zwrotne powiadomienia semafora aplikacji nie może wywołać żadnego interfejsu API ThreadX z opcją zawieszenia.*

### <a name="parameters"></a>Parametry

- **semaphore_ptr** Wskaźnik do wcześniej utworzonego semafora.
- **semaphore_put_notify** Wskaźnik do funkcji powiadomień o przekazaniu semafora aplikacji. Jeśli ta wartość jest TX_NULL, powiadomienie jest wyłączone.
...
- **name_ptr** Wskaźnik na nazwę wątku.
...
- **priorytet** Priorytet liczbowy wątku. Wartości prawne mieszczą się w zakresie od 0 do (TX_MAX_PRIORITES-1), gdzie wartość 0 reprezentuje najwyższy priorytet.
...
- Zakończono tworzenie wątku **TX_SUCCESS** (0x00).
...
- Rozmiar **TX_SIZE_ERROR** (0x05) obszaru stosu jest nieprawidłowy. Wątki muszą mieć co najmniej **TX_MINIMUM_STACK** bajtów do wykonania.
...
- Pomyślnie usunięto wątek **TX_SUCCESS** (0x00).
- **TX_THREAD_ERROR** (0X0E) Nieprawidłowy wskaźnik wątku aplikacji.
- Określony wątek **TX_DELETE_ERROR** (0x11) nie jest w stanie przerwania ani kończenia.
- **TX_CALLER_ERROR** (0X13) Nieprawidłowy obiekt wywołujący tej usługi.
...
> [!NOTE]
> Wywołanie zwrotne powiadomienia o wejściu/wyjściu do wątku aplikacji nie może wywołać żadnego interfejsu API ThreadX z opcją zawieszenia.
...
- **TX_SUCCESS** (0X00) pomyślnie zarejestrowano funkcję powiadamiania o wejściu/wyjściu wątku.
...
### <a name="retuen-values"></a>Retuen wartości
...
### <a name="parameters"></a>Parametry
- **thread_ptr** Wskaźnik do bloku sterowania wątkiem.
- **Nazwa** Wskaźnik do miejsca docelowego dla wskaźnika do nazwy wątku.
- **stan** Wskaźnik do miejsca docelowego dla bieżącego stanu wykonywania wątku. Możliwe wartości są następujące:
    - **TX_READY** (0x00)
    - **TX_COMPLETED** (0x01)
    - **TX_TERMINATED** (0x02)
    - **TX_SUSPENDED** (0x03)
...
> [!IMPORTANT]
> *Biblioteka i aplikacja ThreadX muszą być kompilowane przy użyciu*  * **TX_THREAD_ENABLE_PERFORMANCE_INFO** _ _defined w celu zwrócenia informacji o wydajności przez tę usługę *

### <a name="parameters"></a>Parametry
- **thread_ptr** Wskaźnik do wcześniej utworzonego wątku.
- **wznowień** Wskaźnik do miejsca docelowego dla liczby wznowień tego wątku.
- **zawieszenie** Wskaźnik do miejsca docelowego dla liczby zawieszeń tego wątku.
...
- **priority_inversions** Wskaźnik do miejsca docelowego dla liczby wersji priorytetu tego wątku.
- **time_slices** Wskaźnik do miejsca docelowego dla liczby wycinków czasu tego wątku.
-  braki Wskaźnik do miejsca docelowego dla liczby operacji zwalniania wątku wykonywanych przez ten wątek.
...
- Pobieranie pomyślnej wydajności wątku **TX_SUCCESS** (0x00).
...
*Biblioteka i aplikacja ThreadX muszą być kompilowane przy użyciu*

***TX_THREAD_ENABLE_PERFORMANCE_INFO** _ _defined w celu zwrócenia informacji o wydajności przez tę usługę *

### <a name="parameters"></a>Parametry

- **wznowień** Wskaźnik do miejsca docelowego dla łącznej liczby wznowień wątków.
- **zawieszenie** Wskaźnik do miejsca docelowego dla łącznej liczby zawieszeń wątku.
- **solicited_preemptions** Wskaźnik do miejsca docelowego dla łącznej liczby zastępujący wątków w wyniku wątku wywołującego usługę API ThreadX.
...
-  braki Wskaźnik do miejsca docelowego dla łącznej liczby prób wątków.
...
### <a name="parameters"></a>Parametry
- **thread_ptr** Wskaźnik do wcześniej utworzonego wątku aplikacji.
- **new_threshold** Nowy przechodzenie — poziom priorytetu progu (od 0 do TX_MAX_PRIORITIES-1)).
- **old_threshold** Wskaźnik do lokalizacji, aby przywrócić poprzedni próg zastępujący.

### <a name="return-values"></a>Wartości zwrócone

- Pomyślna zmiana progu **TX_SUCCESS** (0x00).
- **TX_THREAD_ERROR** (0X0E) Nieprawidłowy wskaźnik wątku aplikacji.
...
> [!IMPORTANT]
> * Próg przekroczenia określonego wątku jest automatycznie ustawiany na nowy priorytet. W przypadku pożądanego nowego progu należy użyć usługi ***tx_thread_preemption_change** _ po tym Call._
...
- Pomyślna zmiana priorytetu **TX_SUCCESS** (0x00).
- **TX_THREAD_ERROR** (0X0E) Nieprawidłowy wskaźnik wątku aplikacji.
...
- Pomyślnie zresetowano wątek **TX_SUCCESS** (0x00).
- Określony wątek **TX_NOT_DONE** (0x20) nie jest w stanie **TX_COMPLETED** lub **TX_TERMINATED** .
- **TX_THREAD_ERROR** (0X0E) Nieprawidłowy wskaźnik wątku.
- **TX_CALLER_ERROR** (0X13) Nieprawidłowy obiekt wywołujący tej usługi.
...
- Pomyślna wznowienie wątku **TX_SUCCESS** (0x00).
- **TX_SUSPEND_LIFTED** (0X19) poprzednio ustawić opóźnione zawieszenie zostało zniesione.
- **TX_THREAD_ERROR** (0X0E) Nieprawidłowy wskaźnik wątku aplikacji.
- Określony wątek **TX_RESUME_ERROR** (0x12) nie jest wstrzymany lub został wcześniej zawieszony przez usługę inną niż **_tx_thread_suspend_**.
...
- **timer_ticks** Liczba cykli czasomierza zawieszenia wątku aplikacji wywołującej z zakresu od 0 do 0xFFFFFFFF. Jeśli określono wartość 0, usługa wraca natychmiast.

### <a name="return-values"></a>Wartości zwrócone

- **TX_SUCCESS** (0X00) powodzenie uśpienia wątku.
- Zawieszenie **TX_WAIT_ABORTED** (0x1A) zostało przerwane przez inny wątek, czasomierz lub proces ISR.
- Usługa **TX_CALLER_ERROR** (0x13) wywołana z elementu niebędącego wątkiem.
...
Ta usługa rejestruje funkcję wywołania zwrotnego powiadomienia do obsługi błędów stosu wątków. Gdy ThreadX wykrywa błąd stosu wątku podczas wykonywania, wywoła tę funkcję powiadamiania w celu przetworzenia błędu. Przetwarzanie błędu jest całkowicie zdefiniowane przez aplikację. Wszystkie elementy od zawieszenia naruszenia wątku w celu zresetowania całego systemu mogą zostać wykonane.

> [!IMPORTANT]
> *Biblioteka ThreadX musi być skompilowana przy użyciu* **TX_ENABLE_STACK_CHECKING** *zdefiniowanej w celu zwrócenia informacji o wydajności przez tę usługę.*
...
- Pomyślne zawieszenie wątku **TX_SUCCESS** (0x00).
- **TX_THREAD_ERROR** (0X0E) Nieprawidłowy wskaźnik wątku aplikacji.
...
> [!NOTE]
> *Jest to odpowiedzialność aplikacji, aby upewnić się, że wątek jest w stanie odpowiednim do zakończenia. Na przykład wątek nie powinien być zakończony podczas krytycznego przetwarzania aplikacji lub wewnątrz innych składników oprogramowania pośredniczącego, w którym może to spowodować, że przetwarzanie jest nieznane.*
...
### <a name="return-values"></a>Wartości zwrócone
- Pomyślna przerwanie wątku **TX_SUCCESS** (0x00).
- **TX_THREAD_ERROR** (0X0E) Nieprawidłowy wskaźnik wątku aplikacji.
- **TX_CALLER_ERROR** (0X13) Nieprawidłowy obiekt wywołujący tej usługi.
...
- **TX_SUCCESS** (0x00) pomyślny czas odcięcia.
- **TX_THREAD_ERROR** (0X0E) Nieprawidłowy wskaźnik wątku aplikacji.
...
- **TX_THREAD_ERROR** (0X0E) Nieprawidłowy wskaźnik wątku aplikacji.
- Określony wątek **TX_WAIT_ABORT_ERROR** (0x1B) nie jest w stanie oczekiwania.
...
> [!NOTE]
> *Wygasły czasomierz z jednym zdjęciem musi być resetowany przez*  * **tx_timer_change** _ _before można go aktywować ponownie. *
...
> [!NOTE]
> *Wywołanie*  * przed ponownym uruchomieniem czasomierza jest wymagane **tx_timer_activate** _ _Service. *
...
> [!NOTE]
> *Wygasły czasomierz z jednym zdjęciem musi być resetowany przez* 
* **tx_timer_change** _ _before można go aktywować ponownie. *
...
- **timer_ptr** Wskaźnik do bloku sterowania czasomierzem
- **name_ptr** Wskaźnik na nazwę czasomierza.
...
- **TX_TICK_ERROR** (0X16) Nieprawidłowa wartość (zero) dostarczona dla początkowych taktów.
- Wybrano nieprawidłową aktywację **TX_ACTIVATE_ERROR** (0x17).
- **TX_CALLER_ERROR** (0X13) Nieprawidłowy obiekt wywołujący tej usługi.
...
- **timer_ptr** Wskaźnik do wcześniej utworzonego czasomierza aplikacji.
- **Nazwa** Wskaźnik do miejsca docelowego dla wskaźnika do nazwy czasomierza.
- **aktywne** Wskaźnik do miejsca docelowego dla wskaźnika aktywnego wskazywania czasomierza. Jeśli czasomierz jest nieaktywny lub ta usługa jest wywoływana z samego czasomierza, zwracana jest wartość **TX_FALSE** . W przeciwnym razie, jeśli czasomierz jest aktywny, zwracana jest wartość **TX_TRUE** .
...
- Pobieranie informacji o pomyślnym czasomierzu **TX_SUCCESS** (0x00).
- **TX_TIMER_ERROR** (0X15) Nieprawidłowy wskaźnik czasomierza aplikacji.
...
> [!IMPORTANT]
> *Biblioteka i aplikacja ThreadX muszą być kompilowane przy użyciu*  * **TX_TIMER_ENABLE_PERFORMANCE_INFO** _ _defined do zwrócenia informacji o wydajności przez tę usługę *

### <a name="parameters"></a>Parametry
- **timer_ptr** Wskaźnik do wcześniej utworzonego czasomierza.
- **Aktywuj** Wskaźnik do miejsca docelowego dla liczby żądań aktywacji wykonanych dla tego czasomierza.
- **ponowne aktywowanie** Wskaźnik do miejsca docelowego dla liczby automatycznych ponownych aktywacji wykonanych na tym cyklicznym czasomierzu.
...
> KORYGUJĄC *Dostarczenie TX_NULL dla dowolnego parametru wskazuje, że parametr nie jest wymagany.*

### <a name="return-values"></a>Wartości zwrócone

- Pomyślna wydajność czasomierza **TX_SUCCESS** (0x00).
...
> [!IMPORTANT]
> Aby można było zwracać informacje o wydajności *, biblioteka i aplikacja ThreadX muszą zostać skompilowane z użyciem* **TX_TIMER_ENABLE_PERFORMANCE_INFO** *zdefiniowanej dla tej usługi.*

**Parametry**

- **Aktywuj** Wskaźnik do miejsca docelowego dla łącznej liczby żądań aktywacji wykonanych na wszystkich czasomierzach.
- **ponowne aktywowanie** Wskaźnik do miejsca docelowego dla łącznej liczby automatycznych ponownych aktywacji wykonanych na wszystkich okresowych czasomierzach.
...
- **TX_SUCCESS** (0X00) pomyślne pobieranie wydajności systemu czasomierza.
- **TX_FEATURE_NOT_ENABLED** (0xFF) system nie został skompilowany z włączonymi informacjami o wydajności.
