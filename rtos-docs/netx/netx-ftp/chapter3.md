...
---
# <a name="chapter-3---description-of-azure-rtos-netx-ftp-services"></a>Rozdział 3 — Opis usług FTP usługi Azure RTO NetX

Ten rozdział zawiera opis wszystkich usług FTP usługi Azure RTO NetX (wymienionych poniżej) w kolejności alfabetycznej.
...
- **nx_ftp_client_connect**: *łączenie z serwerem FTP*
- **nx_ftp_client_create**: *Tworzenie wystąpienia klienta FTP*
- **nx_ftp_client_delete**: *usuwanie wystąpienia klienta FTP*
- **nx_ftp_client_directory_create**: *Utwórz katalog na serwerze*
- **nx_ftp_client_directory_default_set**: *Ustaw domyślny katalog na serwerze*
- **nx_ftp_client_directory_delete**: *usuwanie katalogu na serwerze*
- **nx_ftp_client_directory_listing_get**: *Pobieranie listy katalogów z serwera*
- **nx_ftp_client_directory_listing_continue**: *Kontynuuj Wyświetlanie listy katalogów z serwera*
- **nx_ftp_client_disconnect**: *Rozłącz z serwerem FTP*
...
- **nx_ftp_client_file_read**: *Odczytaj z pliku*
- **nx_ftp_client_file_rename**: *Zmień nazwę pliku na serwerze*
- **nx_ftp_client_file_write**: *Zapisz do pliku*
- **nx_ftp_server_create**: *Utwórz serwer FTP*
- **nx_ftp_server_delete**: *usuwanie serwera FTP*
- **nx_ftp_server_start**: *Uruchom serwer FTP*
- **nx_ftp_server_stop**: *ZAtrzymywanie serwera FTP*
...
Ta usługa nawiązuje połączenie utworzonego wcześniej wystąpienia klienta FTP z serwerem FTP przy użyciu podanego adresu IP.
...
- **NX_FTP_NOT_DISCONNECTED**: klient (0xD4) jest już połączony.
...
- NX_PTR_ERROR: (0x16) Nieprawidłowy wskaźnik FTP, adresu IP lub puli pakietów. wskaźnik hasła.
...
- **NX_SUCCESS**: (0X00) pomyślne ustawienie domyślne protokołu FTP.
- **NX_FTP_NOT_CONNECTED**: (0XD3) klient FTP nie jest połączony.
- **NX_FTP_EXPECTED_2XX_CODE**: (0xDA) nie otrzymał odpowiedzi 2xx (ok) 
- NX_PTR_ERROR: (0x07) Nieprawidłowy wskaźnik FTP. 
- NX_CALLER_ERROR: (0x11) Nieprawidłowy obiekt wywołujący tej usługi.
...
Ta usługa pobiera zawartość określonego katalogu na serwerze FTP, który jest połączony z określonym klientem FTP. Dostarczony wskaźnik pakietu będzie zawierać jeden lub więcej wpisów w katalogu. Każdy wpis jest oddzielony przez &lt; kombinację CR/LF \. ***Nx_ftp_client_directory_listing_continue***: należy wywołać, aby ukończyć operację pobierania katalogu.

### <a name="input-parameters"></a>Parametry wejściowe

- **ftp_client_ptr**: wskaźnik do bloku kontroli klienta FTP.
- **directory_name**: Nazwa katalogu, do którego ma zostać pobrana zawartość.
- **packet_ptr**: wskaźnik do docelowego wskaźnika pakietu. Jeśli to się powiedzie, ładunek pakietu będzie zawierać jeden lub więcej wpisów w katalogu.
...
- **NX_FTP_NOT_CONNECTED**: (0XD3) klient FTP nie jest połączony.
- **NX_NOT_ENABLED**: (0x14) nie włączono usługi (IPv6)
...
- **ftp_client_ptr**: wskaźnik do bloku kontroli klienta FTP.
- **packet_ptr**: wskaźnik do docelowego wskaźnika pakietu. Jeśli to się powiedzie, ładunek pakietu będzie zawierać co najmniej jeden wpis w katalogu oddzielony znakiem &lt; CR/LF &gt; .
- **WAIT_OPTION**: określa, jak długo usługa będzie czekać na listę katalogów FTP. Opcje oczekiwania są zdefiniowane w następujący sposób:
  - **wartość limitu czasu**: (od 0X00000001 do 0xFFFFFFFE)
  - **TX_WAIT_FOREVER**: (0Xffffffff) wybranie TX_WAIT_FOREVER powoduje, że wątek wywołujący zawiesza się w nieskończoność do momentu, aż serwer FTP odpowie na żądanie. Wybranie wartości liczbowej (1-0xFFFFFFFE) określa maksymalną liczbę cykli czasomierza, która ma zostać wstrzymana podczas oczekiwania na odpowiedź serwera FTP.
...
- **ftp_client_ptr**: wskaźnik do bloku kontroli klienta FTP.
- **packet_ptr**: wskaźnik do miejsca docelowego dla wskaźnika pakietu danych pobranego z kolejki. Jeśli to się powiedzie, dane pakietu zawierają część lub wszystkie pliki.
- **WAIT_OPTION**: określa, jak długo usługa będzie oczekiwać na odczytanie pliku klienta FTP. Opcje oczekiwania są zdefiniowane w następujący sposób:
  - **wartość limitu czasu**: (od 0X00000001 do 0xFFFFFFFE)
  - **TX_WAIT_FOREVER**: (0Xffffffff) wybranie TX_WAIT_FOREVER powoduje, że wątek wywołujący zawiesza się w nieskończoność do momentu, aż serwer FTP odpowie na żądanie. Wybranie wartości liczbowej (1-0xFFFFFFFE) określa maksymalną liczbę cykli czasomierza, która ma zostać wstrzymana podczas oczekiwania na odpowiedź serwera FTP.

### <a name="return-values"></a>Wartości zwrócone

- **NX_SUCCESS**: (0X00) pomyślne odczytanie pliku FTP.
...
- **passive_mode_enabled**:
  - Jeśli ustawiona na NX_TRUE, tryb pasywny jest włączony.
  - Jeśli ustawiona na NX_FALSE, tryb pasywny jest wyłączony.
...
- **ftp_login**: wskaźnik funkcji do funkcji logowania aplikacji. Ta funkcja jest podana nazwa użytkownika i hasło z klienta żądającego połączenia. Jeśli jest to prawidłowe, funkcja logowania aplikacji powinna zwracać NX_SUCCESS.
- **ftp_logout**: wskaźnik funkcji do funkcji wylogowania aplikacji. Ta funkcja jest podana nazwa użytkownika i hasło z klienta żądającego odłączenia. Jeśli jest to prawidłowe, funkcja logowania aplikacji powinna zwracać NX_SUCCESS.
