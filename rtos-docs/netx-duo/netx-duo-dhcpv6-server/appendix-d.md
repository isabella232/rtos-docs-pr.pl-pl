...
---
# <a name="appendix-d----azure-rtos-netx-duo-advanced-dhcpv6-server-example"></a><span data-ttu-id="76e60-103">Dodatek D — przykład Azure RTO NetX Duo Advanced serwer DHCPv6</span><span class="sxs-lookup"><span data-stu-id="76e60-103">Appendix D -  Azure RTOS NetX Duo Advanced DHCPv6 server example</span></span>

<span data-ttu-id="76e60-104">Jest to zaawansowany serwer DHCPv6, który pokazuje, jak zapisać i pobrać tabelę dzierżawy adresów IP serwera i tabele rekordu klienta z nietrwałej pamięci, zgodnie z wymaganiami RFC 3315.</span><span class="sxs-lookup"><span data-stu-id="76e60-104">This is an advanced DHCPv6 Server which demonstrates saving and retrieving the Server’s IP address lease table and Client record tables from non volatile memory, as required by the RFC 3315.</span></span>

<span data-ttu-id="76e60-105">W tym przykładzie plik dołączany *nxd_dhcpv6_server. h* jest wprowadzany w wierszu 7.</span><span class="sxs-lookup"><span data-stu-id="76e60-105">In this example, the include file *nxd_dhcpv6_server.h* is brought in at line 7.</span></span> <span data-ttu-id="76e60-106">Następnie w poniższym przykładowym kodzie zostanie utworzony wątek aplikacji serwera DHCPv6 NetX Duo w wierszu 81.</span><span class="sxs-lookup"><span data-stu-id="76e60-106">Next, the NetX Duo DHCPv6 Server application thread is created at line 81 in the example code below.</span></span> <span data-ttu-id="76e60-107">Należy zauważyć, że blok sterowania DHCPv6 "*dhcp_server_0*" został zdefiniowany jako zmienna globalna w wierszu 19 wcześniej.</span><span class="sxs-lookup"><span data-stu-id="76e60-107">Note that the DHCPv6 control block “*dhcp_server_0*” was defined as a global variable at line 19 previously.</span></span>

<span data-ttu-id="76e60-108">Przed utworzeniem wystąpienia serwera DHCPv6 NetX Duo, demonstracja tworzy pulę pakietów do wysyłania komunikatów protokołu DHCPv6 w wierszu 84, tworzy interfejs wątku IP w wierszu 102 i włącza protokół UDP w NetX Duo w wierszu 116.</span><span class="sxs-lookup"><span data-stu-id="76e60-108">Before creating the NetX Duo DHCPv6 Server instance, the demo creates packet pool for sending DHCPv6 messages in line 84, creates an IP thread interface in line 102, and enables UDP in NetX Duo in line 116.</span></span>

<span data-ttu-id="76e60-109">Pomyślne utworzenie serwera DHCPv6 w programie NetX Duo w wierszu 136 obejmuje dwie opcjonalne funkcje wywołania zwrotnego opisane w rozdziale 1.It to IPv6 i ICMPv6 wymagane przez NetX Duo do przetwarzania operacji IPv6 i DHCPv6 w wierszu 162-163.</span><span class="sxs-lookup"><span data-stu-id="76e60-109">The successful creation of NetX Duo DHCPv6 Server in line 136 includes the the two optional callbacks function described in Chapter 1.It enables IPv6 and ICMPv6 necessary for NetX Duo to process IPv6 and DHCPv6 operations in line 162-163.</span></span> <span data-ttu-id="76e60-110">Aby wątek serwera DHCPv6 był gotowy do uruchomienia, serwer DHCPv6 musi sprawdzić poprawność adresu IPv6 (167-180) i zdefiniować swój interfejs DHCPv6 w wierszach 208-209.</span><span class="sxs-lookup"><span data-stu-id="76e60-110">Before the DHCPv6 Server thread is ready to run, the DHCPv6 server must validate its IPv6 address(167-180),and define its DHCPv6 interface in lines 208-209.</span></span> <span data-ttu-id="76e60-111">Usługa *nx_dhcpv6_set_server_duid* jest wywoływana, aby utworzyć serwer, jeśli żaden identyfikator DUID serwera nie został wcześniej utworzony w wierszu 266.</span><span class="sxs-lookup"><span data-stu-id="76e60-111">The *nx_dhcpv6_set_server_duid* service is called to create the Server if no Server DUID has been previously created in line 266.</span></span> <span data-ttu-id="76e60-112">Serwer konfiguruje zakres adresów IP na potrzeby tworzenia listy adresów, które można przypisać.</span><span class="sxs-lookup"><span data-stu-id="76e60-112">The Server sets up an IP address range for creating a list of assignable addresses.</span></span> <span data-ttu-id="76e60-113">Jeśli dane są zapisywane z poprzedniej sesji, pobiera rekordy klienta i dane dzierżawy IPv6 z pamięci w wierszach 283-318.</span><span class="sxs-lookup"><span data-stu-id="76e60-113">If data is saved from a previous session, it retrieves Client records and IPv6 lease data from memory in lines 283-318.</span></span> <span data-ttu-id="76e60-114">Tworzy również identyfikator DUID serwera lub, jeśli został wcześniej utworzony, pobiera dane identyfikatora DUID z magazynu określonego przez użytkownika.</span><span class="sxs-lookup"><span data-stu-id="76e60-114">It also creates its Server DUID, or if one was previously created, retrieves the DUID data from user specified storage.</span></span> <span data-ttu-id="76e60-115">Jest to konieczne do odtworzenia spójnego identyfikatora DUID serwera w ramach ponownych uruchomień.</span><span class="sxs-lookup"><span data-stu-id="76e60-115">This is necessary to reproduce a consistent Server DUID across reboots.</span></span> <span data-ttu-id="76e60-116">Opcjonalnie aplikacja hosta definiuje serwer DNS dla klientów żądających konfiguracji serwera DNS.</span><span class="sxs-lookup"><span data-stu-id="76e60-116">Optionally the host application defines a DNS server for Clients requesting DNS server configuration.</span></span>

<span data-ttu-id="76e60-117">Następnie Host uruchamia DHCPv6Server w wierszu 329.</span><span class="sxs-lookup"><span data-stu-id="76e60-117">Next, the host starts the DHCPv6Server in line 329.</span></span> <span data-ttu-id="76e60-118">Spowoduje to utworzenie gniazda UPD serwera DHCPv6 i aktywowanie czasomierzy serwera DHCPv6 NetX Duo.</span><span class="sxs-lookup"><span data-stu-id="76e60-118">This creates the DHCPv6 Server UPD socket and activates NetX Duo DHCPv6 Server timers.</span></span> <span data-ttu-id="76e60-119">Następnie serwer czeka na odebranie żądań klientów.</span><span class="sxs-lookup"><span data-stu-id="76e60-119">Then the Server waits to receive Client requests.</span></span> <span data-ttu-id="76e60-120">Chociaż może ona obsłużyć wielu klientów, może jednocześnie przetwarzać tylko jedno żądanie klienta.</span><span class="sxs-lookup"><span data-stu-id="76e60-120">While it can service many Clients it can only process a single Client request at a time.</span></span>

<span data-ttu-id="76e60-121">Pozostała część tego przykładu zawiera implementacje hostów do zapisywania i pobierania tabel serwera z przypisaną pulą adresów IPv6 i rekordów klientów do i z pamięci nieulotnej.</span><span class="sxs-lookup"><span data-stu-id="76e60-121">The remainder of the example contains host implementations for saving and retrieving Server tables of its assignable IPv6 address pool and Client records to and from non volatile memory respectively.</span></span> <span data-ttu-id="76e60-122">Zawiera również procedurę obsługi dla opcji żądanych przez klientów DHCPv6, które nie są obsługiwane bezpośrednio przez serwer DHCPv6 NetX Duo (tylko opcja serwera DNS jest obecnie obsługiwana).</span><span class="sxs-lookup"><span data-stu-id="76e60-122">It also contains an option handler for options requested by DHCPv6 Clients that are not supported directly by the NetX Duo DHCPv6 Server (only the DNS server option is currently supported).</span></span> <span data-ttu-id="76e60-123">Poniżej znajduje się kod demonstrujący sposób zapisywania i pobierania "nietrwały czas", za pomocą którego serwer śledzi przydzielone wygaśnięcie dzierżawy IP.</span><span class="sxs-lookup"><span data-stu-id="76e60-123">Lastly there is a code for demonstrating how to save and retrieve ‘non volatile time’ by which the Server keeps track of assigned IP lease expiration.</span></span>
...
<span data-ttu-id="76e60-124">**Rysunek 6. Zaawansowana aplikacja serwera DHCPv6 NetX Duo**</span><span class="sxs-lookup"><span data-stu-id="76e60-124">**Figure 6. Advanced NetX Duo DHCPv6 Server Application**</span></span>