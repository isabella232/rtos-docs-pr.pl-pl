---
title: Dodatek A — Azure RTOS powrotu/błędu netx secure
...
ms.locfileid: "116791758"
---
# <a name="appendix-a---azure-rtos-netx-secure-returnerror-codes"></a>Dodatek A — Azure RTOS powrotu/błędu netx secure

## <a name="netx-secure-tls-return-codes"></a>Kody powrotne bezpiecznego TLS netX

Tabela 1 poniżej zawiera listę możliwych kodów błędów, które mogą być zwracane przez Azure RTOS NetX Secure TLS. Należy pamiętać, że usługi mogą również zwracać kody błędów TCP/IP — wartości protokołu TLS zaczynają się od 0x101, a wartości TCP/IP są 0x100. Wartości zwracane X.509 zaczynają się od 0x181. Zapoznaj się z dokumentacją NetX TCP/IP, aby uzyskać informacje na temat zwracanych wartości TCP/IP, a poniżej przedstawiono wartości X.509.
...
| NX_SECURE_TLS_INVALID_EPOCH                        | 0x11E  | Odebrano komunikat DTLS i dopasowano go do sesji USŁUGI DTLS, ale miał on nieprawidłową epokę i należy go zignorować.                                                   |
...
| NX_SECURE_DTLS_SESSION_NOT_FOUND                  | 0x124  | Wywołujący wyszukał sesję DTLS, ale dany adres IP i port nie są zgodne z żadnymi wpisami w pamięci podręcznej.                                             |
...
| NX_SECURE_TLS_MISSING_CRYPTO_ROUTINE              | 0x13B  | Podczas próby wykonania operacji kryptograficznych jeden z wpisów w tabeli ciphersuite (lub jeden z jej wskaźników funkcji) został nieprawidłowo ustawiony na wartość NULL. |
| NX_SECURE_TLS_EMPTY_EC_GROUP                      | 0x13C  | Szyfrowanie ECC jest ustawione, ale nie jest obsługiwana grupa EC.                                                                                                             |
| NX_SECURE_TLS_EMPTY_EC_POINT_FORMAT              | 0x13D  | Szyfrowanie ECC jest ustawione, ale nie jest obsługiwany format punktu EC.                                                                                                      |
| NX_SECURE_TLS_BAD_SERVERHELLO_KEYSHARE            | 0x13E  | W rozszerzeniu TLS 1.3 KeyShare z serwera zdalnego serwer podał coś, czego się nie spodziewaliśmy.                                                         |
| NX_SECURE_TLS_INSUFFICIENT_METADATA_SPACE         | 0x13F  | Dostarczone przez aplikację "metadane" dla procedur kryptograficznych TLS były zbyt małe.                                                                             |
| NX_SECURE_TLS_POST_HANDSHAKE_RECEIVED             | 0x140  | Nie jest to błąd, ale wskazanie kontynuowania przetwarzania do momentu odbierania danych aplikacji.                                                                    |
| NX_SECURE_TLS_BAD_CLIENTHELLO_KEYSHARE            | 0x141  | W rozszerzeniu TLS 1.3 KeyShare z klienta zdalnego klient podał coś, czego się nie spodziewaliśmy.                                                         |
| NX_SECURE_TLS_1_3_UNKNOWN_CIPHERSUITE            | 0x142  | Odebrano nieznany szyfrsuite podczas korzystania z TLS 1.3.                                                                                                              |
| NX_SECURE_TLS_INVALID_SESSION_TICKET              | 0x143  | Odebrano komunikat NewSessionTicket z nieprawidłowymi lub nieprawidłowymi parametrami.                                                                                      |
| NX_SECURE_TLS_MISSING_EXTENSION                    | 0x144  | W komunikacie pominięto określone rozszerzenie.                                                                                                                  |
| NX_SECURE_TLS_CERTIFICATE_REQUIRED                 | 0x145  | Serwer odebrał pusty certyfikat.                                                                                                                         |
| NX_SECURE_TLS_UNEXPECTED_CLIENTHELLO               | 0x146  | Serwer TLS 1.3 odbiera usługę ClientHello do ponownego negocjowania.                                                                                                         |
| NX_SECURE_TLS_INAPPROPRIATE_FALLBACK               | 0x147  | Klient zdalny próbował niewłaściwej zmiany wersji TLS na starszą.                                                                                               |
| NX_SECURE_TLS_BAD_CLIENTHELLO_PSK_EXTENSION      | 0x148  | W rozszerzeniu TLS 1.3 PSK od klienta zdalnego klient podał coś, czego się nie spodziewaliśmy.                                                              |
| NX_SECURE_TLS_PSK_BINDER_MISMATCH                 | 0x149  | W rozszerzeniu TLS 1.3 PSK od klienta zdalnego klient podał złą wartość binderu PSK.                                                                  |
| NX_SECURE_TLS_CRYPTO_KEYS_TOO_LARGE              | 0x14A  | Podczas próby wygenerowania kluczy sesji TLS bufor kluczy był zbyt mały — zwiększ liczbę NX_SECURE_TLS_KEY_MATERIAL_SIZE.                                     |
| NX_SECURE_TLS_UNSUPPORTED_ECC_CURVE               | 0x14B  | Host zdalny dostarczył certyfikat lub wybrał szyfr z krzywą ECC, która nie jest obsługiwana.                                                         |
| NX_SECURE_TLS_UNSUPPORTED_ECC_FORMAT              | 0x14C  | Napotkano typ krzywej lub format ECC, który nie jest obsługiwany.                                                                                                 |
| NX_SECURE_TLS_UNSUPPORTED_SIGNATURE_ALGORITHM     | 0x14D  | Napotkano nieobsługiwany algorytm podpisu (używany w sytuacjach wymiany kluczy lub innych sytuacji bez certyfikatu).                                                |
| NX_SECURE_TLS_SIGNATURE_VERIFICATION_ERROR        | 0x14E  | Sprawdzanie weryfikacji podpisu nie powiodło się (używane w sytuacjach wymiany kluczy lub innych sytuacji bez certyfikatu).                                                                    |
| NX_SECURE_TLS_UNEXPECTED_MESSAGE                   | 0x14F  | Usługa TLS odebrała nieoczekiwany komunikat z hosta zdalnego.                                                                                                      |
| NX_SECURE_TLS_AEAD_DECRYPT_FAIL                   | 0x150  | Przychodzący rekord nie przejdzie sprawdzania integralności za pomocą szyfrów AEAD.                                                                                            |
| NX_SECURE_TLS_RECORD_OVERFLOW                      | 0x151  | Odebrano rekord TLSCiphertext o zbyt długiej długości.                                                                                                   |
| NX_SECURE_TLS_HANDSHAKE_FRAGMENT_RECEIVED         | 0x152  | Odebrano pofragmentowany komunikat handshake — podjąć odpowiednie działania na wyższym poziomie maszyny stanu.                                                     |
...
| NX_SECURE_TLS_SEND_ADDRESS_MISMATCH               | 0x122  | Element wywołujący próbował wysłać dane za pośrednictwem sesji DTLS z adresem IP lub portem, które nie są zgodne z sesją.                                                  |
| NX_SECURE_TLS_NO_FREE_DTLS_SESSIONS              | 0x123  | Nowe połączenie próbowało pobrać sesję DTLS z pamięci podręcznej, ale nie było żadnych bezpłatnych.                                                                        |
| NX_SECURE_DTLS_SESSION_NOT_FOUND                  | 0x124  | Wywołujący wyszukiwał sesję DTLS, ale dany adres IP i port nie były zgodne z żadnymi wpisami w pamięci podręcznej.                                             |
| NX_SECURE_TLS_NO_MORE_PSK_SPACE                  | 0x125  | Wywołujący próbował dodać psk do sesji TLS, ale nie było więcej miejsca w danej sesji.                                                          |
| NX_SECURE_TLS_NO_MATCHING_PSK                     | 0x126  | Host zdalny podał wskazówkę tożsamości psk, która nie pasuje do żadnego z naszych magazynów lokalnych.                                                                         |
| NX_SECURE_TLS_CLOSE_NOTIFY_RECEIVED               | 0x127  | Sesja TLS odebrała alert CloseNotify z hosta zdalnego wskazujący, że sesja została ukończona.                                                           |
| NX_SECURE_TLS_NO_AVAILABLE_SESSIONS               | 0x128  | Sesje TLS w obiekcie TLS nie są dostępne do obsługi połączenia.                                                                                         |
| NX_SECURE_TLS_NO_CERT_SPACE_ALLOCATED            | 0x129  | Nie przydzielono miejsca na certyfikat dla przychodzących certyfikatów zdalnych.                                                                                          |
| NX_SECURE_TLS_PADDING_CHECK_FAILED                | 0x12A  | Dopełnienie szyfrowania w komunikacie przychodzącym nie było poprawne.                                                                                                    |
| NX_SECURE_TLS_UNSUPPORTED_CERT_SIGN_TYPE         | 0x12B  | Podczas przetwarzania obiektu CertificateVerifyRequest serwer zdalny nie dostarczył obsługiwanego typu certyfikatu.                                                    |
| NX_SECURE_TLS_UNSUPPORTED_CERT_SIGN_ALG          | 0x12C  | Podczas przetwarzania żądania CertificateVerifyRequest serwer zdalny nie dostarczył obsługiwanego algorytmu podpisu.                                                 |
| NX_SECURE_TLS_INSUFFICIENT_CERT_SPACE             | 0x12D  | Za mało miejsca w buforze certyfikatu przydzielonego dla certyfikatu.                                                                                              |
| NX_SECURE_TLS_PROTOCOL_VERSION_CHANGED            | 0x12E  | Wersja protokołu w przychodzącym rekordzie protokołu TLS nie jest dopasowana do wersji ustanowionej sesji.                                                          |
| NX_SECURE_TLS_NO_RENEGOTIATION_ERROR              | 0x12F  | Odebrano komunikat HelloRequest, ale nie będziemy negocjować ponownie.                                                                                           |
...
| NX_SECURE_TLS_EMPTY_REMOTE_CERTIFICATE_RECEIVED  | 0x132  | Host zdalny wysłał pusty komunikat certyfikatu.                                                                                                            |
| NX_SECURE_TLS_RENEGOTIATION_EXTENSION_ERROR       | 0x133  | Wystąpił błąd podczas przetwarzania lub wysyłania rozszerzenia oznaczania bezpiecznej renegocji.                                                                      |
| NX_SECURE_TLS_RENEGOTIATION_SESSION_INACTIVE      | 0x134  | Próba renegocjowania sesji była próbą z sesją TLS, która nie była aktywna.                                                                                 |
| NX_SECURE_TLS_PACKET_BUFFER_TOO_SMALL            | 0x135  | TLS odebrał rekord, który był zbyt duży dla przypisanego buforu pakietów. Nie można przetworzyć rekordu.                                                    |
| NX_SECURE_TLS_EXTENSION_NOT_FOUND                 | 0x136  | Określone rozszerzenie nie zostało odebrane z hosta zdalnego podczas ugody TLS.                                                                          |
| NX_SECURE_TLS_SNI_EXTENSION_INVALID               | 0x137  | TLS odebrało nieprawidłowe Oznaczanie nazwy serwera rozszerzenia.                                                                                                      |
| NX_SECURE_TLS_CERT_ID_INVALID                     | 0x138  | Aplikacja próbowała dodać certyfikat serwera z nieprawidłową wartością identyfikatora certyfikatu (prawdopodobnie 0).                                                                 |
| NX_SECURE_TLS_CERT_ID_DUPLICATE                   | 0x139  | Aplikacja próbowała dodać certyfikat serwera z identyfikatorem certyfikatu już obecnym w magazynie lokalnym.                                                        |
| NX_SECURE_TLS_RENEGOTIATION_FAILURE                | 0x13A  | Host zdalny nie podał rozszerzenia oznaczania bezpiecznej renegocjacji ani pseudoszyfrowania SCSV, więc nie można przeprowadzić bezpiecznej renegocjacji.      |
| NX_SECURE_TLS_MISSING_CRYPTO_ROUTINE              | 0x13B  | Podczas próby wykonania operacji kryptograficznych jeden z wpisów w tabeli ciphersuite (lub jeden z jej wskaźników funkcji) został nieprawidłowo ustawiony na wartość NULL. |
| NX_SECURE_TLS_EMPTY_EC_GROUP                      | 0x13C  | Szyfrowanie ECC jest ustawione, ale nie jest obsługiwana grupa EC.                                                                                                             |
| NX_SECURE_TLS_EMPTY_EC_POINT_FORMAT              | 0x13D  | Szyfrsuite ECC jest ustawiony, ale nie jest obsługiwany format punktu EC.                                                                                                      |
| NX_SECURE_TLS_BAD_SERVERHELLO_KEYSHARE            | 0x13E  | W rozszerzeniu TLS 1.3 KeyShare z serwera zdalnego serwer podał coś, czego się nie spodziewaliśmy.                                                         |
| NX_SECURE_TLS_INSUFFICIENT_METADATA_SPACE         | 0x13F  | Dostarczone przez aplikację "metadane" dla procedur kryptograficznych TLS były zbyt małe.                                                                             |
| NX_SECURE_TLS_POST_HANDSHAKE_RECEIVED             | 0x140  | Nie jest to błąd, ale wskazanie kontynuowania przetwarzania do momentu odbierania danych aplikacji.                                                                    |
| NX_SECURE_TLS_BAD_CLIENTHELLO_KEYSHARE            | 0x141  | W rozszerzeniu TLS 1.3 KeyShare z klienta zdalnego klient podał coś, czego się nie spodziewaliśmy.                                                         |
...
| NX_SECURE_TLS_AEAD_DECRYPT_FAIL                   | 0x150  | Przychodzący rekord nie przejdzie sprawdzania integralności za pomocą szyfrów AEAD.                                                                                            |
| NX_SECURE_TLS_RECORD_OVERFLOW                      | 0x151  | Odebrano rekord TLSCiphertext o zbyt długiej długości.                                                                                                   |
| NX_SECURE_TLS_HANDSHAKE_FRAGMENT_RECEIVED         | 0x152  | Odebrano pofragmentowany komunikat uściślinia — podjąć odpowiednie działanie na wyższym poziomie maszyny stanu.                                                     |
...
| Nazwa błędu                                   | Wartość | Opis                                                                                                        |
| ------------------------------------------------ | --------- | ---------------------------------------------------------------------------------------------------------------------- |
| NX_SECURE_X509_SUCCESS                        | 0x00      | Stan pomyślnego powrotu. (Tak samo jak NX_SUCCESS)                                                                        |
...
| NX_SECURE_X509_FOUND_NON_ZERO_PADDING      | 0x183     | Oczekiwano dopełnienia o wartości 0 — coś się różni.                                                               |
...
| NX_SECURE_PKCS1_INVALID_PRIVATE_KEY         | 0x18A     | Przekazano plik klucza prywatnego PKCS#1, ale formatowanie było niepoprawne.                                            |
...
| NX_SECURE_X509_INVALID_VERSION               | 0x18F     | Certyfikat zawierał pole, które nie jest zgodne z tą wersją.                                           |
| NX_SECURE_X509_INVALID_TAG_CLASS            | 0x190     | Certyfikat zawierał tag ASN.1 z nieprawidłową wartością klasy tagu.                                                   |
| NX_SECURE_X509_INVALID_EXTENSIONS            | 0x191     | Certyfikat zawiera rozszerzenia TLV, ale nie zawiera sekwencji.                                          |
| NX_SECURE_X509_INVALID_EXTENSION_SEQUENCE   | 0x192     | Certyfikat zawierał nieprawidłową sekwencję rozszerzeń X.509.                                                   |
| NX_SECURE_X509_CERTIFICATE_EXPIRED           | 0x193     | Certyfikat miał pole "nie po", które było mniejsze niż bieżący czas.                                             |
| NX_SECURE_X509_CERTIFICATE_NOT_YET_VALID   | 0x194     | Certyfikat zawierał pole "nie wcześniej", które było większe niż bieżący czas.                                         |
...
| NX_SECURE_X509_CRL_ISSUER_MISMATCH          | 0x197     | Dostarczone listy CRL i certyfikat nie zostały wystawione przez ten sam urząd certyfikacji.                                      |
| NX_SECURE_X509_CRL_SIGNATURE_CHECK_FAILED  | 0x198     | Sprawdzanie podpisu listy CRL nie powiodło się względem wystawcy.                                                                       |
| NX_SECURE_X509_CRL_CERTIFICATE_REVOKED      | 0x199     | Certyfikat został znaleziony w prawidłowej listy CRL i dlatego został odwołany.                                                 |
