...
Ta usługa tworzy podkatalog w bieżącym katalogu domyślnym lub w ścieżce podanej w nazwie katalogu. W przeciwieństwie do katalogu głównego podkatalogi nie mają ograniczenia liczby plików, które mogą przechowywać. Katalog główny może zawierać tylko liczbę wpisów określaną przez rekord rozruchowy.
...
> [!WARNING]
> *Jeśli używasz ścieżki innej niż lokalna, ważne jest, aby uniemożliwić innym wątkom aplikacji (ze zmianą na poziomie semafora, mutexu lub priorytetu ThreadX) zmianę tego katalogu w innych wątkach aplikacji podczas przechodzenia przez katalog. W przeciwnym razie mogą zostać uzyskane nieprawidłowe wyniki.*
...
Ta usługa zmienia nazwę katalogu na określoną nową nazwę katalogu. Zmiana nazwy jest również wykonywana względem określonej ścieżki lub ścieżki domyślnej. Jeśli ścieżka jest określona w nowej nazwie katalogu, nazwa katalogu zostanie skutecznie przeniesiona do określonej ścieżki. Jeśli ścieżka nie zostanie określona, nazwa katalogu zostanie umieszczona w bieżącej ścieżce domyślnej.
...
- **memory_size:** rozmiar pamięci na początku. Aby zapewnić odporność na uszkodzenia, rozmiar pamięci na początku musi być co najmniej 3072 bajty i musi mieć wielokrotność rozmiaru sektora.
...
Aby przydzielić miejsce o pojemności spoza 4 GB, aplikacja musi używać usługi *fx_file_extended_best_effort_allocate*.
...
- **FX_SUCCESS** (0x00) Alokacja plików z najlepszymi rozwiązaniami.
- **FX_ACCESS_ERROR** (0x06) Określony plik nie jest otwarty do zapisu.
- **FX_NOT_OPEN** (0x07) Określony plik nie jest obecnie otwarty.
...
Ta usługa zamyka określony plik. Jeśli plik został otwarty do zapisu, a jeśli został zmodyfikowany, ta usługa kończy proces modyfikacji pliku, aktualizując jego wpis katalogu przy użyciu nowego rozmiaru oraz bieżącej daty i czasu systemowego.
...
> [!WARNING]
> *Ta usługa tworzy plik o zerowej długości, tj. nie przydzielone klastry. Alokacja odbywa się automatycznie przy kolejnych zapisach plików lub może być wykonywana z wyprzedzeniem za pomocą usługi fx_file_allocate lub fx_file_extended_allocate dla miejsca spoza 4 GB).*
...
> [!WARNING]
> *Należy zachować ostrożność przy obcinaniu plików, które mogą być również jednocześnie otwarte do odczytu. Obcinanie pliku również otwartego do odczytu może spowodować odczytanie nieprawidłowych danych.*
...
- **FX_SUCCESS** (0x00) Przerwanie nośnika pomyślnego.
- **FX_MEDIA_NOT_OPEN** (0x11) Określony nośnik nie jest otwarty.
- **FX_PTR_ERROR** (0x18) Nieprawidłowy wskaźnik nośnika.
- **FX_CALLER_ERROR** (0x20) nie jest wątkiem.
...
- **FX_FAT_READ_ERROR** (0x03) Błąd podczas odczytywania informacji o nośniku FAT.
- **FX_IO_ERROR** (0x90) sterownika We/Wy.
...
- **media_ptr:** Wskaźnik do bloku sterowania multimediami.
- **media_open_notify:** Funkcja wywołania zwrotnego powiadamiania o otwarciu nośnika do zainstalowania. Przekazanie wartości NULL jako funkcji wywołania zwrotnego powoduje wyłączenie otwartego wywołania zwrotnego multimediów.
...
Ta usługa zapisuje podany bufor w określonym sektorze logicznym.
...
Ta usługa określa długość podanej nazwy Unicode. Znak Unicode jest reprezentowany przez dwa bajty. Nazwa Unicode jest serią dwóch bajtów znaków Unicode zakończonych przez dwa bajty NULL (dwa bajty o wartości 0).
...
> [!IMPORTANT]
> *Ta usługa jest taka sama **fx_unicode_short_name_get()**, z tą różnicą, że obiekt wywołujący dostarcza rozmiar buforu docelowego jako argument wejściowy. Dzięki temu usługa gwarantuje, że krótka nazwa nie przekroczy buforu docelowego.*
